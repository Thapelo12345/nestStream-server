...
async function SeriesGetUrl(browser, pageUrl, title, season, episode) {
    ...
    await mainPage.waitForSelector("#search-input", { state: "visible" });
...
async function UpdateMovie(browser, pageUrl, title) {
    browser = await chromium.launch({ headless: true });
    const context = await browser.newContext();
    const mainPage = await context.newPage();
    // opening upLink to page
    ...
    // await retryAction(async () => {await Promise.all([mainPage.waitForSelector(".detail-title", { state: "attached" }), card.click()])}, 4, 15000 );
    await card.click();
    console.log("Waitting for season button to be visible!....");
    await mainPage.waitForSelector("#watch-btn", { state: "visible" });
    console.log("Click Watch btn!.");
    const watchBtn = await mainPage.locator("#watch-btn");
    await watchBtn.click();
    await mainPage.waitForSelector("#watch-dropdown", { state: "visible" });
    ...
    const playLink = await mainPage.locator("#player-iframe").getAttribute("src");
    console.log("Updating Cloud!.");
    await Movies.updateOne({ movieHeader: title }, {
        $set: { playingUrl: playLink }
    });
    return playLink;
} //end of update movie function
async function FindShow(browser, pageUrl, title, typeOfShow) {
    browser = await chromium.launch({ headless: true });
    const context = await browser.newContext();
    const mainPage = await context.newPage();
    // opening upLink to page
    ...
    await searchBtn.click();
    await mainPage.waitForSelector(".page-btn", { state: "hidden" });
    await retryAction(async () => { await mainPage.waitForSelector(".show-card, .movie-card"); }, 2, 1500);
    ...
    if (startingCardPosition === -1)
        throw new Error("Their No MORE SHOWS!..");
    ...
    await retryAction(async () => {
        await Promise.all([mainPage.waitForSelector(".detail-content", { state: "attached" }), card.click()]);
    }, 2, 15000);
    ...
    const genres = $2(".genre-tag").toArray().map((genre) => $2(genre).text().trim());
    ...
    const cast = $2(".cast-card").toArray().map((cas) => {
    ...
    if (typeOfShow === "series") {
        ...
        let seasons;
        seasons = $2("#season-dropdown .dropdown-item").toArray().map((season) => $2(season).text().trim().split("-")[0].trim());
        let newSeason;
        ...
        const waitingSeasons = seasons.filter((season) => {
...
async function AddSeason(browser, pageUrl, title) {
