...
async function webSiteScrapper(browser, pageUrl, showType) {
    ...
    for (let page = 1; page <= maxPages; page++) {
        const cardsVisible = await mainPage.locator(".show-card, .movie-card").first().isVisible();
        if (!cardsVisible)
            await mainPage.waitForSelector(".show-card, .movie-card", { state: "visible" });
        const number_of_cards = await mainPage.locator(".show-card, .movie-card").count();
        for (let i = 0; i < number_of_cards; i++) {
            if (!await mainPage.locator(".show-card, .movie-card").first().isVisible())
                await mainPage.waitForSelector(".show-card, .movie-card", { state: "visible" });
            let activePageText = await mainPage.locator(".page-btn.active").innerText();
            // moving to current page
            if (activePageText !== String(page)) {
                console.log(`MOVING TO PAGE: ${page}\nCurrently on PAGE: ${activePageText}\n`);
                // starting a do while loop
                do {
                    const firstOldCard = await mainPage.locator(".show-card, .movie-card").first();
                    const titleLocation = firstOldCard.locator(".show-info h3, .movie-info h3");
                    ...
                    activePageText = await mainPage.locator(".page-btn.active").innerText();
            ...
            const cardName = await card.locator(".show-info .show-title, .movie-info .movie-title").textContent();
            // search cloud for card
            const cloudShow = showType == "series"
            ...
            await retryAction(async () => {
                await Promise.all([mainPage.waitForSelector(".detail-title", { state: "attached" }), card.click()]);
            }, 4, 15000);
            ...
            const showLanguage = showType == "series" ? $2(".extra-info .info-item").eq(1).find(".info-value").text().trim() : $2(".extra-info .info-item")
                .last().find(".info-value").text().trim();
            ...
            if (showType == "series" && targetGenres.some((genre) => genres.includes(genre))) {
            ...
            let seasons = showType == "series" ? $2("#season-dropdown .dropdown-item").toArray().map((season) => $2(season).text().trim().split("-")[0].trim()) : [];
            ...
            if (showType == "series") {
                ...
                const newSeason = await gettingTheSeason(mainPage, seasons[0], page3Url, page3);
